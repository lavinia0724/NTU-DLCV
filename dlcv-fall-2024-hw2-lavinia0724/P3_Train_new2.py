...
def main():
    ...
    model.cond_stage_model.tokenizer.add_tokens('<new2>')
    new_token_id = model.cond_stage_model.tokenizer.convert_tokens_to_ids('<new2>')
    ...
    prompts = [
        "An illustration of a whimsical forest in the style of <new2>",
        "A fantasy adventure scene featuring characters in the style of <new2>",
        "A vibrant and magical landscape in the style of <new2>",
        "An adventurous journey with fantastic creatures in the style of <new2>",
        "A mystical setting full of magic, painted in the style of <new2>",
        "A playful scene of animals and adventurers in the style of <new2>",
        "A lush enchanted forest depicted in the style of <new2>",
        "A beautiful fantasy artwork inspired by the style of <new2>",
        "An imaginative tale with characters in a magical setting, all in the style of <new2>",
        "A colorful and detailed fantasy illustration, rendered in the style of <new2>"
    ]

    # Load DavidRevoy images for training
    image_paths = glob.glob(r"./hw2_data/textual_inversion/1/*.jpg")
    assert len(image_paths) == 5, "There should be exactly 5 David images."
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            epoch_embedding_path = os.path.join(opt.outdir, f"DavidRevoy_epoch{epoch + 1}.pt")
    ...
    embedding_path = os.path.join(opt.outdir, "DavidRevoy.pt")
