...
def main():
    ...
    model.cond_stage_model.tokenizer.add_tokens('<new3>')
    new_token_id = model.cond_stage_model.tokenizer.convert_tokens_to_ids('<new3>')
    ...
    prompts = [
        "A portrait of a graceful <new3> with green eyes",
        "A lovely shot of a relaxed <new3>",
        "A cute <new3> sitting by the window",
        "A charming <new3> with a curious look",
        "A serene image of <new3> resting comfortably",
        "An adorable <new3> in a cozy indoor setting",
        "A detailed photo capturing the calm nature of <new3>",
        "A beautiful image of <new3> sitting elegantly",
        "A playful <new3> interacting with its surroundings",
        "A cute gray <new3> looking inquisitive"
    ]

    # Load cat images for training
    image_paths = glob.glob(r"./hw2_data/textual_inversion/cat/*.jpg")
    assert len(image_paths) == 5, "There should be exactly 5 cat images."
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            epoch_embedding_path = os.path.join(opt.outdir, f"cat_epoch{epoch + 1}.pt")
    ...
    embedding_path = os.path.join(opt.outdir, "cat.pt")
